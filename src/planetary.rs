...
//!
//! Implements Keplerian orbital elements with secular variations from Meeus
//! (*Astronomical Algorithms*, Table 31.A) and perturbation corrections for
//! Jupiter, Saturn, and Uranus. Accuracy is approximately 1 arcminute for
//! dates within ±2000 years of J2000.0.
...
// ---------------------------------------------------------------------------
// Orbital elements at J2000.0 and their rates per century
// ---------------------------------------------------------------------------

/// Mean orbital elements for a planet: (value at J2000.0, rate per Julian century).
/// Elements: L (mean longitude), a (semi-major axis AU), e (eccentricity),
/// i (inclination), Ω (longitude of ascending node), ϖ (longitude of perihelion).
...
/// Orbital elements from Meeus Table 31.A (valid for J2000.0 ± ~2000 years).
fn orbital_elements(planet: Planet) -> Result<OrbitalElements> {
    match planet {
...
///
/// Uses Newton-Raphson iteration. `m_rad` is mean anomaly in radians,
/// `e` is eccentricity. Returns E in radians.
fn solve_kepler(m_rad: f64, e: f64) -> f64 {
    let mut ea = m_rad + e * m_rad.sin(); // initial guess
    for _ in 0..50 {
...
fn heliocentric_position(planet: Planet, jd: f64) -> Result<(f64, f64, f64)> {
    let t = julian_centuries(jd);
    let elts = orbital_elements(planet)?;
...
fn earth_heliocentric(jd: f64) -> (f64, f64, f64) {
    ...
    // Earth's ascending node is 0 by definition (ecliptic plane)
    let lon = v + peri;
...
pub fn compute_position(planet: Planet, jd: f64) -> Result<PlanetaryPosition> {
    ...
    let (h_lon, h_lat, h_r) = heliocentric_position(planet, jd)?;
    let (e_lon, _e_lat, e_r) = earth_heliocentric(jd);

    let h_lon_rad = deg_to_rad(h_lon);
    let h_lat_rad = deg_to_rad(h_lat);
    let e_lon_rad = deg_to_rad(e_lon);

    // Convert to rectangular heliocentric ecliptic coordinates
    let x_p = h_r * h_lat_rad.cos() * h_lon_rad.cos() - e_r * e_lon_rad.cos();
    let y_p = h_r * h_lat_rad.cos() * h_lon_rad.sin() - e_r * e_lon_rad.sin();
    let z_p = h_r * h_lat_rad.sin();
...
mod tests {
    ...
    fn positions_vary_over_time() {
        // Positions should change day to day
